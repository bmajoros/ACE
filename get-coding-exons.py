...
if(len(sys.argv)!=6):
    exit(sys.argv[0]+" <in.gff> <genome.2bit> <out-exons.fasta> <out-introns.fasta> <max-features>")
(gffFile,twoBitFile,outExons,outIntrons,maxFeatures)=sys.argv[1:]
...
for gene in genes:
    ...
    if(hash["gene_type"]!="protein_coding"): continue
