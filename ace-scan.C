...
#include "EnumerateAltStructures_Sim.H"
...
private:
  ...
  String configFile, refGffFile, refFasta, outGff, outACE;
  Set<String> seen;
  ...
  void handleCoding(GffTranscript *altTrans,
		    ...
		    const Labeling &projectedLab);
  void flushOutput(ostream &osACE,const bool &mapped);
  ...
  void simulateBroken(ProjectionChecker &,
		      TranscriptSignals *,
		      GffTranscript *altTrans,
		      ostream &osACE);
};
...
int ACE::main(int argc,char *argv[])
{
  ...
  loadInputs(configFile,refGffFile,refFasta,refFasta);
  ...
  bool mapped=false;
  if(referenceIsOK && refTrans->isCoding()) {
...
void ACE::parseCommandLine(const CommandLine &cmd)
{
  if(cmd.numArgs()!=5)
    throw String("\n\
ace-scan <ace.config> <ref.gff> <ref.fasta> <out.gff> <outfile>\n\
			  ...
			  ostream &osACE)
{
  ...
  // ### SIMULATE BROKEN SITES
  if(refTrans->isCoding()) simulateBroken(checker,signals,altTrans,osACE);
...
void ACE::simulateBroken(ProjectionChecker &checker,TranscriptSignals *signals,
			 GffTranscript *altTrans,ostream &osACE)
{
  const int numSignals=signals->numSignals();
  for(int i=1 ; i<numSignals-1 ; ++i) {
    TranscriptSignal &signal=(*signals)[i];
    String key=substrate+" "+signal.pos;
	...
	bool LOF=false;
	if(s.proteinFate==NMD_NONE) {
	  if(refTrans->isCoding()) {
	    String refProtein, altProtein;
	    checker.translate(*refTrans,*s.transcript,refProtein,altProtein);
	    const bool proteinsDiffer=refProtein!=altProtein;
	    if(proteinsDiffer) {
	      int matches;
	      alignProteins(refProtein,altProtein,matches);
	      int mismatches=refProtein.length()-matches;
	      if(mismatches>10) LOF=true;
	    }
	  }
	}
	else LOF=true;
    ...
    osACE<<substrate<<"\t"<<signal.pos<<"\t"<<!allLOF<<endl;
    signal.broken=false;
  }
}
 ...
 ****************************************************************/
void ACE::processAltStructure(AlternativeStructure &s,
			      Essex::CompositeNode *altStructNode,
			      const Labeling &projectedLab)
{
...
/****************************************************************
 ACE::alignProteins()
 ****************************************************************/
float ACE::alignProteins(const String &refStr,const String &altStr,
			 int &matches)
{
