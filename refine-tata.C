...
BOOM::Set<BOOM::String> consensuses;
const char *consensusStr[]=
{
  "TATAAA",
  "TATATA",
  "CATAAA", // ### might want to comment this out
  "CATATA"  // ### might want to comment this out
};
...
class Application
{
  ...
  void processForwardFeature(int featureBegin,const BOOM::String &seq);
  void processReverseFeature(int featureEnd,const BOOM::String &seq);
...
int Application::main(int argc,char *argv[])
  {
    ...
    if(cmd.numArgs()!=7)
      throw string("\n\
refine-tata <seed-model> <genes.gff> <contigs.fasta>\n\
...
");
    ...
    int numCons=sizeof(consensusStr)/sizeof(consensusStr[0]);
    for(int i=0 ; i<numCons ; ++i) consensuses.insert(consensusStr[i]);
  ...
  while(fastaReader.nextSequence(def,seq))
      ...
      if(transcriptMap.isDefined(substrate))
	{
	  ...
	  for(int i=0 ; i<n ; ++i)
		...
		{
		case '+':
		  processForwardFeature(feature->getBegin(),seq);
		  break;
		case '-':
		  processReverseFeature(feature->getEnd(),seq);
		  break;
		default:
		  continue;
		}
...
void Application::processForwardFeature(int featureBegin,
...
void Application::processReverseFeature(int featureEnd,
					const BOOM::String &seq)
{
...
void Application::updateModel(bool echo,float keepFraction)
{
  ...
  for(int i=0 ; i<numMargins ; ++i)
      ...
      for(int pos=0 ; pos<numPositions ; ++pos)
	{
	  ...
	  if(!consensuses.isMember(consensus)) continue;
